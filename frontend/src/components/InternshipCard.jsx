...
export default function InternshipCard({ internship }) {
  ...
  return <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-card">
    <div className="flex items-start justify-between gap-3"><div><p className="text-sm font-bold text-leaf">{internship.sector}</p><h2 className="mt-1 text-xl font-extrabold text-navy">{internship.job_title}</h2><p className="mt-1 font-semibold text-slate-600">{internship.company_name}</p></div><MatchBadge percentage={internship.match_percentage} /></div>
    <div className="mt-5 grid grid-cols-2 gap-3 text-sm"><p>📍 {internship.city}, {internship.state}</p><p>₹{internship.stipend.toLocaleString('en-IN')} {t('card.perMonth')}</p><p>🗓 {t('card.months', { count: internship.duration })}</p></div>

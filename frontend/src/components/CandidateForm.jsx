...
export default function CandidateForm({ profile, onChange, onNext }) {
  ...
  return <section className="page-shell"><div className="panel mx-auto max-w-2xl">
    ...
    <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">{educationOptions.map((item) => <button type="button" key={item} onClick={() => onChange({ education: item })} className={`chip ${profile.education === item ? 'chip-selected' : ''}`}>{t('options.'+item, {defaultValue: item})}</button>)}</div>
    ...
    <select value={profile.branch} onChange={(event) => onChange({ branch: event.target.value })} className="mt-3 min-h-12 w-full rounded-xl border-2 border-slate-300 bg-white px-3 text-base">
      <option value="">{t('profile.chooseSubject')}</option>{branchOptions.map((item) => <option key={item} value={item}>{t('options.'+item, {defaultValue: item})}</option>)}
    </select>
